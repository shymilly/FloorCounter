...
// Структура для хранения параметров детекции (ОПТИМИЗИРОВАННЫЕ)
struct DetectionParams {
    double angleThreshold = 8.0;           // Увеличен угол
    double minLengthRatio = 0.15;          // Уменьшен порог длины линии
    int cannyThreshold1 = 30;              // Понижен для лучшей чувствительности
    int cannyThreshold2 = 100;             // Понижен
    int houghThreshold = 60;               // Понижен
    int morphKernelSize = 15;              // Уменьшен
    int minLineGap = 20;                   // Минимальный разрыв между линиями
    int minLineLength = 50;                // Минимальная длина линии
};

// Улучшенная функция группировки Y-координат в кластеры с весами
int clusterYCoordinates(vector<int>& yCoordinates, vector<double>& weights, double threshold) {
...
// Улучшенная морфологическая обработка для горизонтальных линий
Mat morphologicalProcessing(const Mat& edges, int kernelSize) {
    ...
    Mat closingKernel = getStructuringElement(MORPH_RECT, Size(kernelSize, 1));
    morphologyEx(morphed, morphed, MORPH_CLOSE, closingKernel);
    
    return morphed;
}

// Поиск горизонтальных линий с оценкой уверенности
...
// Улучшенное автоматическое определение границ здания
...
int detectFloors(const Mat& image, vector<int>& floorPositions, Mat& outputImage) {
    ...
    int distanceThreshold = max(10, typicalGap / 2);
    
    vector<int> clusteredY = yCoordinates;
    vector<double> clusteredWeights = weights;
    
    int numClusters = clusterYCoordinates(clusteredY, clusteredWeights, distanceThreshold);
    
    floorPositions = clusteredY;
    sort(floorPositions.begin(), floorPositions.end());
    
    for (int& pos : floorPositions) {
        pos += buildingRect.y;
    }
    
    int numFloors = numClusters + 1;
    
    if (floorPositions.size() >= 2 && building.rows > 0) {
        int totalHeight = building.rows;
        int avgSpacing = totalHeight / (numFloors + 1);
        
        if (avgSpacing > typicalGap * 1.8 && numFloors < 20) {
            int additionalFloors = min(10, totalHeight / typicalGap - numFloors);
            if (additionalFloors > 0) {
                numFloors += additionalFloors;
            }
        }
    }
    
    if (numFloors < 1) numFloors = 1;
    if (numFloors > 35) numFloors = 35;
    ...
    if (floorPositions.size() >= 2 && numFloors > floorPositions.size() + 1) {
        int avgGap = 0;
        for (size_t i = 1; i < floorPositions.size(); ++i) {
            avgGap += floorPositions[i] - floorPositions[i-1];
        }
        avgGap /= (floorPositions.size() - 1);
        
        int firstLine = floorPositions[0];
        int lastLine = floorPositions.back();
        
        for (int y = firstLine - avgGap; y > buildingRect.y; y -= avgGap) {
            if (y > buildingRect.y + 20) {
                line(outputImage, Point(0, y), Point(outputImage.cols - 1, y), Scalar(0, 255, 255), 1);
            }
        }
        
        for (int y = lastLine + avgGap; y < buildingRect.y + buildingRect.height; y += avgGap) {
            if (y < buildingRect.y + buildingRect.height - 20) {
                line(outputImage, Point(0, y), Point(outputImage.cols - 1, y), Scalar(0, 255, 255), 1);
            }
        }
    }
    ...
    string debugText = "Lines found: " + to_string(horizontalLines.size()) + ", Clusters: " + to_string(numClusters);
